module Mateable
    ...
    class Blockchain < Peatio::Blockchain::Abstract
      ...
      rescue Mateable::Client::Error => e
      ...
      def fetch_transaction(transaction)
        ...
        transaction_hash.fetch('vout').select do |entry|
          ...
          if transaction.to_address == entry['scriptPubKey']['address'] && entry.fetch('value').to_d == transaction.amount
      ...
      def transaction_sources(transaction)
        ...
        transaction_hash['vin'].each_with_object([]) do |vin, source_addresses|
          ...
          source_addresses << source['scriptPubKey']['address']
        end.compact.uniq
      end
      ...
      def client
        @client ||= Mateable::Client.new(settings_fetch(:server))
      end
